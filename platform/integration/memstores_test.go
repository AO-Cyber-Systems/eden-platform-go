...
func (s *memHouseholdStore) CreateHouseholdWithOwner(_ context.Context, h household.Household, owner household.Member) (household.Household, household.Member, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	now := time.Now().UTC()
	h.ID = uuid.New()
